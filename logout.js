function setSession(authResult) {
    ...
    localStorage.setItem("expires_at", 1546281000000)
}
function logout() {
...
function handleAuthentication(callback) {
    webAuth.parseHash(function(err, authResult) {
        authResult && authResult.accessToken && authResult.idToken ? (window.location.hash = "",
        setSession(authResult),
        "function" == typeof callback && callback.call(this, authResult)) : err && console.log(err)
    })
}
...
function getErrorHash(hash) {
    return params = hash.slice(1).split("&"),
    message = {},
    $.each(params, function(index, value) {
        kv = value.split("="),
        message[kv[0]] = decodeURI(kv[1])
    }),
    message
}
